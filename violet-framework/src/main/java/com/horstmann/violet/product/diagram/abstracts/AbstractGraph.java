...
public abstract class AbstractGraph implements Serializable, Cloneable, IGraph {
	...
	public INode findNode(Point2D p) {
		for (INode n : getAllNodes()) {
			...
			Rectangle2D boundsToCheck = new Rectangle2D.Double(locationOnGraph.getX(), locationOnGraph.getY(),
	...
	public boolean addNode(INode newNode, Point2D p) {
		...
		if (potentialParentNode != null) {
			...
			Point2D relativeLocation = new Point2D.Double(p.getX() - parentLocationOnGraph.getX(),
					p.getY() - parentLocationOnGraph.getY());
	...
	List<INode> selfRecursiveNodes = new ArrayList<>();
	Map<INode, List<INode>> checkBiDirectionalList = new HashMap<>();
	public static boolean isFeature1Enabled = true;
	public static boolean isFeature2Enabled = true;
	...
	public boolean connect(IEdge e, INode start, Point2D startLocation, INode end, Point2D endLocation,
		...
		if (!isBiDirectional && (isSelfRecursive && !selfRecursiveNodes.contains(start) || !isSelfRecursive)) {
			...
			if (null != start && start.addConnection(e)) {
				...
				if (end != null) {
					...
					if (!start.getId().getValue().equals(end.getId().getValue())) {
						if (checkBiDirectionalList.containsKey(start))
							checkBiDirectionalList.get(start).add(end);
						else {
							List<INode> nodes = new ArrayList<>();
							nodes.add(end);
							checkBiDirectionalList.put(start, nodes);
						}
					}
				}
				if (isSelfRecursive)
					selfRecursiveNodes.add(start);
				return true;
			}
		} else {
			if (isSelfRecursive)
				...
				JOptionPane.showMessageDialog(null, "WARNING", "Warning", JOptionPane.INFORMATION_MESSAGE);
		}
		return false;
	}
	...
	public void removeEdge(IEdge... edgesToRemove) {
		for (IEdge anEdgeToRemove : edgesToRemove) {
			...
			if (startingNode.getId().getValue().equals(endingNode.getId().getValue()))
				selfRecursiveNodes.remove(startingNode);
			if (checkBiDirectionalList.containsKey(startingNode)
					&& checkBiDirectionalList.get(startingNode).contains(endingNode))
				checkBiDirectionalList.get(startingNode).remove(endingNode);
